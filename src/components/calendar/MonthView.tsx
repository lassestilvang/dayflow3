  ...
  const getItemsForDate = (date: Date) => {
    ...
    const filteredTasks = tasks.filter((task) => {
      ...
      if (optimisticTask) {
        // If there's an optimistic update, check if it's still on this date
        return (
          optimisticTask.scheduledDate &&
          isSameDay(optimisticTask.scheduledDate, date)
        );
      }
      return true; // No optimistic update, keep original
    });
    ...
    return {
      events: [
        ...filteredEvents.map((e) => getOptimisticItem(e) as Event),
        ...optimisticEventsForDate,
      ],
      tasks: [
        ...filteredTasks.map((t) => getOptimisticItem(t) as Task),
        ...optimisticTasksForDate,
      ],
    };
  };
      ...
      if (draggedData && dropZoneId.startsWith("date-")) {
        ...
        if ("category" in item) {
          ...
          setOptimisticItems((prev) =>
            new Map(prev).set(item.id, optimisticTask),
          );
          ...
          // Apply optimistic update immediately
          setOptimisticItems((prev) =>
            new Map(prev).set(item.id, optimisticEvent),
          );
    ...
    const calculateMaxItems = () => {
      if (gridRef.current) {
        ...
        const availableItemHeight =
          availableHeight / 6 - dateNumberHeight - cellPadding; // Divide by 6 rows
        const maxItems = Math.max(
          1,
          Math.floor(availableItemHeight / itemHeight),
        );
    ...
    <>
      <div className="h-[calc(100vh-4rem)] overflow-hidden bg-background p-4">
        <div className="max-w-7xl mx-auto h-full flex flex-col">
          ...
          <div
            ref={gridRef}
            className="grid grid-cols-7 gap-px bg-border flex-1 auto-rows-fr"
          >
            {calendarDays.map((date, index) => {
              if (!date) {
                return (
                  <div
                    key={`empty-${index}`}
                    className="bg-muted/50 min-h-full"
                  />
                );
              }
            ...
            {"category" in draggedItem ? (
              <div
                className={cn(
                  ...
                  draggedItem.category === "personal" &&
                    "bg-orange-100 text-orange-800",
                  draggedItem.category === "travel" &&
                    "bg-purple-100 text-purple-800",
                  draggedItem.category === "inbox" &&
                    "bg-gray-100 text-gray-800",
                  draggedItem.completed && "opacity-60 line-through",
                )}
              >
                <Checkbox
                  checked={draggedItem.completed}
                  className="size-3"
                  disabled
                />
                {draggedItem.title}
              ...
              <div
                className="text-xs p-1 rounded truncate h-6 flex items-center"
                style={{
